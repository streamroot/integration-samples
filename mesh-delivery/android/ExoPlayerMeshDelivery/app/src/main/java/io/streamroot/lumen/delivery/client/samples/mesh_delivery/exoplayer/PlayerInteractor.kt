...
class ExoPlayerBandwidthMeter(private val context: Context, playerBuilder: ExoPlayer.Builder) :
    BandwidthMeter, TransferListener {
    data class HandlerAndListener(
        val handler: Handler,
        val eventListener: BandwidthMeter.EventListener
    )
    ...
    fun setDriver(driver: Driver) {
        ...
        if (driver == Driver.PLAYER) {
            ...
            listeners.forEach {
                defaultBandwidthMeterSR.addEventListener(
                    it.handler,
                    it.eventListener
                )
            }
...
class PlayerInteractor(
    ...
    override fun bufferHealth() =
        runSyncOnEPHandler { (player.bufferedPosition - player.currentPosition) / 1000.0 }!!
...
private abstract class LoadControlBufferTargetBridge(protected val loadControl: LoadControl) :
    ...
    protected fun LoadControl.getAccessibleFieldElseThrow(fieldName: String) = runCatching {
        ...
        ?: throw IllegalArgumentException("Impossible to retrieve field `$fieldName` value from LoadControl of type `${this::class.java.simpleName}`")
    ...
    protected fun LoadControl.getLongFromFieldElseThrow(fieldName: String) = runCatching {
        ...
        ?: throw IllegalArgumentException("Impossible to retrieve long `$fieldName` value from LoadControl of type `${this::class.java.simpleName}`")
...
private object BufferTargetBridgeFactory {
    fun createInteractor(loadControl: LoadControl, audioOnly: Boolean = false): BufferTargetBridge {
        return runCatching { LoadControlBufferTargetBridgeV1(loadControl) }.getOrNull()
            ?.also { Log.v("Misc", "[Lumen] Using interactor V1") }
            ?: runCatching { LoadControlBufferTargetBridgeV2(loadControl, audioOnly) }.getOrNull()
                ?.also { Log.v(TAG, "[Lumen] Using interactor V2") }
